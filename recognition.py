...
def predict_by_frame(X_frame, knn_clf=None, model_path=None, distance_threshold=0.5):
    """
    Recognizes faces in given image using a trained KNN classifier
...
def show_prediction_labels_on_image_by_frame(frame, predictions):
    """
    Shows the face recognition results visually.
...
def run():
    ...
    while 1 > 0:
        ...
        if ret:
            ...
            predictions = predict_by_frame(img, knn_clf=knn_clf)
            frame = show_prediction_labels_on_image_by_frame(frame, predictions)
